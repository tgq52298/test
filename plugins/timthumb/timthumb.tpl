...
if(! defined('MEMORY_LIMIT') )				define ('MEMORY_LIMIT', '_MEMORY_LIMITM');							// Set PHP memory limit
if(! defined('BLOCK_EXTERNAL_LEECHERS') ) 	define ('BLOCK_EXTERNAL_LEECHERS', _BLOCK_EXTERNAL_LEECHERS);				// If the image or webshot is being loaded on an external site, display a red "No Hotlinking" gif.
...
if(! defined('FILE_CACHE_TIME_BETWEEN_CLEANS'))	define ('FILE_CACHE_TIME_BETWEEN_CLEANS', _FILE_CACHE_TIME_BETWEEN_CLEANS);	// How often the cache is cleaned

if(! defined('FILE_CACHE_MAX_FILE_AGE') ) 	define ('FILE_CACHE_MAX_FILE_AGE', _FILE_CACHE_MAX_FILE_AGE);				// How old does a file have to be to be deleted from the cache
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.timthumb.txt');			// What to put at the end of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'timthumb');				// What to put at the beg of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', '_FILE_CACHE_DIRECTORY');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
if(! defined('MAX_FILE_SIZE') )				define ('MAX_FILE_SIZE', _MAX_FILE_SIZE);						// 10 Megs is 10485760. This is the max internal or external file size that we'll process.
if(! defined('CURL_TIMEOUT') )				define ('CURL_TIMEOUT', _CURL_TIMEOUT);							// Timeout duration for Curl. This only applies if you have Curl installed and aren't using PHP's default URL fetching mechanism.
if(! defined('WAIT_BETWEEN_FETCH_ERRORS') )	define ('WAIT_BETWEEN_FETCH_ERRORS', 3600);				// Time to wait between errors fetching remote file

//Browser caching
if(! defined('BROWSER_CACHE_MAX_AGE') ) 	define ('BROWSER_CACHE_MAX_AGE', _BROWSER_CACHE_MAX_AGE);				// Time to cache in the browser
if(! defined('BROWSER_CACHE_DISABLE') ) 	define ('BROWSER_CACHE_DISABLE', false);				// Use for testing if you want to disable all browser caching

//Image size and defaults
if(! defined('MAX_WIDTH') )					define ('MAX_WIDTH', _MAX_WIDTH);								// Maximum image width
if(! defined('MAX_HEIGHT') )				define ('MAX_HEIGHT', _MAX_HEIGHT);							// Maximum image height
if(! defined('NOT_FOUND_IMAGE') )			define ('NOT_FOUND_IMAGE', '_NOT_FOUND_IMAGE');							// Image to serve if any 404 occurs
if(! defined('ERROR_IMAGE') )				define ('ERROR_IMAGE', '_ERROR_IMAGE');								// Image to serve if an error occurs instead of showing error message
...
if(! defined('DEFAULT_Q') )					define ('DEFAULT_Q', _DEFAULT_Q);								// Default image quality. Allows overrid in timthumb-config.php
if(! defined('DEFAULT_ZC') )				define ('DEFAULT_ZC', _DEFAULT_ZC);								// Default zoom/crop setting. Allows overrid in timthumb-config.php
if(! defined('DEFAULT_F') )					define ('DEFAULT_F', '');								// Default image filters. Allows overrid in timthumb-config.php
if(! defined('DEFAULT_S') )					define ('DEFAULT_S', _DEFAULT_S);								// Default sharpen value. Allows overrid in timthumb-config.php
