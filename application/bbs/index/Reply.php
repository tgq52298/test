...
use app\bbs\model\Reply AS ReplyMode;
...
//             }
            ...
            if (in_wap()) { //判断来自哪个手机
                if (!table_field('bbs_reply','phone_type')) {
                    query("ALTER TABLE  `qb_bbs_reply` ADD  `phone_type` VARCHAR( 50 ) NOT NULL COMMENT  '发表来自什么手机'");
                }
                $data['phone_type'] = fun('client@phone_type');
            }
    ...
    private function ajax_content($name='',$page='',$pagename='', $aid=0 ,$rows=0,$order='',$by='',$status='',$data_type=''){
        ...
        $info = $this->topic_model->getInfoByid($aid);      //主题信息
        $id = $aid;
    ...
    public function get_list($aid=0,$rows=0,$status=0,$order='',$by='',$page=0){
        ...
        $listdb->each(function($rs,$key){
            ...
            $rs['picurls'] = $this->get_pic($rs['picurl']);
            ...
            if($rs['reply']){
                ...
                foreach ($_children AS $k=>$v){
                    $_children[$k]['content'] = fun('Content@bbscode',$_children[$k]['content']);
                    $_children[$k]['picurls'] = $this->get_pic($v['picurl']);
    ...
    protected function get_pic($pics=''){
        if ($pics=='') {
            return [];
        }
        $listdb = [];
