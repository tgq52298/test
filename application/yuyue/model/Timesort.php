...
class Timesort extends Model
{
    ...
    protected function initialize()
    {
        ...
        $this->table = self::$table_pre.self::$model_key.'_timesort';
    }
    
    public static function getsort($uid=0){
        $array = self::where('uid',$uid?:login_user('uid'))->find();
        if ($array){
            return json_decode($array['name'],true)?:['默认'];
        }else{
            return ['默认'];
        }
    }
